...
exports.paySucess= (req, res) =>{ 
    ...
    if(transactionId && paymentType == 'RTGS'){
        ...
        db.sequelize.query('select c.FNAME as "depositorName",cp.PHONE_NUMBER as "phoneNumber",FE_PAY_TYPE as "paymentType",\
		PRODUCT_ID as "productId",CUSTOMER_ID as "customerId",PERIOD as "period",INT_PAY_FREQUENCY as "interestPayment",\
		CATEGORY_ID as "categoryId",RATE_OF_INT as "rateOfInterest", DEPOSIT_AMT as "depositAmount",\
		MATURITY_AMOUNT as "maturityAmount" from PG_RTGS_NEFT_TRANS_DETAILS PR left join customer c on PR.customer_id = c.cust_id join cust_phone cp on\ PR.customer_id = cp.cust_id where TRANSACTION_ID = :transactionId',
            ...
            if(results.length > 0){
				...
				var depositorName = results[0].depositorName; // dd/mm/yyyy hh:mm:ss
			 ...
			 var msg = urlencode('Dear '+firstName+', You have generated RTGS/NEFT payment advice for new fixed deposit request with transaction ID '+transactionId+'. TNPFCL will issue Fixed Deposit confirmation receipt on realisation of payment as per payment advice generated.');
                ...
                return res.status(200).send({"responseCode":200,
                    ...
                    "paymentReference":PaymentReference,"response":results,"date": date,
					"maturityAmount":maturityAmount,"depositorName":depositorName
                }); 
            } else {
            ...
            db.sequelize.query('select ACCT_NUM as "depositNumber", FE_PAY_TYPE as "paymentType", PRODUCT_ID as "productId",CUSTOMER_ID as "customerId",PERIOD as "period",INT_PAY_FREQUENCY as "interestPayment", CATEGORY_ID as "categoryId",RATE_OF_INT as "rateOfInterest" from PG_RTGS_NEFT_TRANS_DETAILS where TRANSACTION_ID = :transactionId',
                { replacements: {transactionId: transactionId}, type: sequelize.QueryTypes.SELECT }
            ).then(results =>{
                console.log(results);
                if(results.length > 0){
                    ...
                    var rateOfInterest = results[0].rateOfInterest;			 
			            db.sequelize.query('select * from  API_fd_summary where accountnumber =:depositNumber',
                            { replacements: {depositNumber: depositNumber}, type: sequelize.QueryTypes.SELECT }
                                ...
                                return res.status(200).send({"responseCode":200,"paymentType":paymentType,"productId":productId, "customerId":customerId,"period":period,
