			...
			if(results.length == 0){
				db.sequelize.query('select depositClosure (:customerId, :depositNumber, :purpose, :renewCloseFdUrl, :channelId, :agentId, :closureType) as acknowledgementId from dual',
					...
					db.sequelize.query(query,{replacements:{customerId: customerId}, type: sequelize.QueryTypes.SELECT}
							...
							let msg = urlencode('Dear '+depositorName+', Your Service Request No. '+acknowledgementId+' for Deposit Closure has been received by TN Power Finance for processing');
							let data = 'username=' + username + '&hash=' + hash + '&sender=' + sender + '&numbers=' + phoneNumber + '&message=' + msg;
